...
def guess_number(max_number, max_attempts)
  ...
  loop do
    ...
    if attempts > max_attempts
      puts "The winning number is #{winning_number}.\nBetter luck next time." # Added this line to make the program 
      break
    end
    ...
    if guess == winning_number
      puts 'Yes! You win.'
      break # add a break if the correct number is guessed
    else
      puts 'Nope. Too small.' if guess < winning_number
      puts 'Nope. Too big.'   if guess > winning_number

      # Try again:
      # guess_number(max_number, max_attempts) ----removed this line
    end
  end
end
